...
def screen_setup(token):
    while True:
        ...
        print("\n  Starting session...", end="", flush=True)
...
def _print_hud(session, q_num=None):
    ...
    print(f"  {q_tag}{topic.title()}  ({lang})")
...
def screen_question(challenge, q_num, session):
    ...
    print()
    print("  Commands: [s]tats  [n]ew topic  [r]eset  [q]uit")
    return q, qtype
...
def game_loop(token, session, lesson_ctx):
    ...
    while True:
        ...
        if cmd in ("s", "stats"):
            ...
            q_num -= 1   # don't count stats command as a question
            continue
        ...
        result    = r2.json()
        prev_diff, updated = screen_feedback(result, prev_diff,
        ...
        print("  [Enter] next question  "
              "[n] new topic  [s] stats  [r] reset  [q] quit")
        cmd = prompt().lower()
    ...
    while True:
        ...
        while True:
            ...
            if action == "quit":
                ...
                break  # go back to setup_session

            elif action == "reset":
                # re-show lesson for same topic, same session
                lesson_ctx = screen_lesson(session)
                # continue inner loop
