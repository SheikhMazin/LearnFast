...
def create_session(language: str, topic: str, starting_difficulty: int = 2) -> dict:
    return {
        ...
        # curriculum fields — populated by generate_curriculum after creation
        "curriculum":          [],
        "current_node":        0,
        "node_stats":          {},
    }
...
def update_session(
    ...
    session["avg_time_seconds"] = (
        sum(session["time_history"]) / len(session["time_history"])
    )
    ...
    session["difficulty"] = adjust_difficulty(
        session["difficulty"], _stats_for_difficulty(session)
    )
    return session


def _stats_for_difficulty(session: dict) -> dict:
    return {
...
def get_session_stats(session: dict) -> dict:
    ...
    return {
        ...
        "total_nodes":         len(session.get("curriculum", [])),
    }
...
def reset_session(session: dict) -> dict:
    ...
    session["last_updated_at"]     = _now_iso()
    # Reset curriculum traversal but keep the curriculum map itself
    session["current_node"]        = 0
    session["node_stats"]          = {}
    for node in session.get("curriculum", []):
        node["status"] = "in_progress" if node["id"] == 0 else "locked"
    return session
